...
Welcome to this comprehensive guide on abstract interpretation combined with Binary Decision Diagrams (BDDs). Whether you're a student encountering formal methods for the first time, a practitioner seeking to apply verification techniques, or a researcher exploring new analysis approaches, this guide is designed to meet you where you are and take you further.

== Who This Guide Is For

This guide serves multiple audiences, each with different backgrounds and goals:

*Complete Beginners* #reading-path(path: "beginner"): If you're new to program verification and formal methods, start with Part I. We'll build intuition through concrete examples before introducing mathematical formalism. No prior knowledge of abstract interpretation or BDDs is assumed—just basic programming experience and curiosity.

*Practitioners* #reading-path(path: "implementation"): If you're a software engineer interested in applying verification techniques to real systems, focus on Part I (Chapters 1-5) for conceptual understanding, then jump to Part II (Chapter 12) for implementation details and benchmarks.

*Researchers and Students* #reading-path(path: "advanced"): If you're already familiar with program analysis basics, you might skim Part I and dive into Part II, which provides complete mathematical foundations, proofs, and connections to research literature.
...
- *Tells a story*: We build concepts progressively, motivating each idea before formalizing it
- *Shows real code*: Every concept is backed by working Rust implementations in the bdd-rs library
- *Balances rigor and intuition*: We maintain mathematical precision while providing accessible explanations
...
*Part I: Gentle Introduction (Chapters 1-6)* starts from first principles, building intuition about program abstraction, control flow, and symbolic representations. Through running examples like heater controllers and traffic lights, we motivate why BDD-based path-sensitive analysis matters. This part is accessible to anyone with programming background.

*Part II: Deep Dive (Chapters 7-15)* provides complete theoretical foundations, formal proofs, implementation techniques, and research connections. This part enables readers to understand the mathematics deeply and implement their own analyses.
...
1. Start with the *Prologue* to understand the motivation
2. Work through *Part I* sequentially, trying the examples
3. When you encounter formalism in Part II, don't panic—we build intuition first
4. Use the *exercises* to test your understanding
5. Refer to *appendices* when you need background material
...
This guide builds on decades of research in abstract interpretation, pioneered by Patrick and Radhia Cousot, and symbolic verification using BDDs, advanced by Randal Bryant and many others. We're grateful to the formal methods community for creating the foundations on which this work stands.

The bdd-rs library and this guide are open-source projects, welcoming contributions from the community. Thank you to everyone who has contributed code, feedback, and insights.

== Let's Begin

Program verification is a journey from informal reasoning to mathematical certainty. This guide will equip you with powerful tools for analyzing programs automatically, guaranteeing properties that testing alone cannot ensure.
